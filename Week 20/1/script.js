...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    for (let hero of heroes) {
        heroContent += `<div>${returnInfo(this, index)}
        ${rating(index)}</div> `;
        console.log(this);
        index++;
    }
    document.getElementById("info").innerHTML = heroContent;
...
function returnInfo(e, index) {
    heroContent = `<div class='superhero' id="superhero_${index}"'>
        ${mainInfo(e, index)}
        </div>`
        return heroContent;
}

function mainInfo(e, index) {
    const mainContent = `<h2>${heroes[index].name}</h2>
        <div style="background-image: url(clip.png)" class="btn" onclick="changeInfo(this, ${index})"></div>
        <div><b class="q_title">Вселенная:</b> <span class="font">${heroes[index].universe}</span></div>
...
function print(e, index) {
    const heroContent = mainInfo(e, index);
    document.getElementById(`superhero_${index}`).innerHTML = heroContent;

}

function rating(index) {
    let rating = `<div class="rating-area">
